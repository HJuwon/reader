...
type UseEpisodeRulesParams = {
	...
	episodeRuleManagerOpen: boolean;
	setParsedNovel: (
		novel: ParsedNovel
	) => void;
...
export function useEpisodeRules({
	...
	const [
		episodeRulesLoading,
		setEpisodeRulesLoading,
	] = useState(false);

	const [
		episodeRuleSaving,
		setEpisodeRuleSaving,
	] = useState(false);
	...
	const reparseWithRules =
		useCallback(
			(rules: EpisodeRule[]) => {
				...
				if (
					typeof selectedEpisodeStartLine ===
					"number"
				) {
					const nextIndex =
