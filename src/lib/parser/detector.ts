...
const patterns: Pattern[] = [
  ...
  },
  {
    ...
    regex: /^\s*(?:chapter|ch\.?)\s*[-.]?\s*(\d{1,5})(?:\s*[-.:·]\s*|\s+)?(.*?)\s*$/i,
    getNumber: (m) => Number(m[1]),
    getTitle: (m) => m[2]?.trim() || "",
  },
  {
    name: "english_episode",
    regex: /^\s*(?:episode|ep\.?)\s*[-.]?\s*(\d{1,5})(?:\s*[-.:·]\s*|\s+)?(.*?)\s*$/i,
    getNumber: (m) => Number(m[1]),
    getTitle: (m) => m[2]?.trim() || "",
  },
  {
    name: "part",
    regex: /^\s*(?:part|pt\.?)\s*[-.]?\s*(\d{1,5})(?:\s*[-.:·]\s*|\s+)?(.*?)\s*$/i,
    getNumber: (m) => Number(m[1]),
    getTitle: (m) => m[2]?.trim() || "",
  },
  {
    name: "bracket_episode",
    regex: /^\s*[\[【]\s*(?:제\s*)?(\d{1,5})\s*화\s*[\]】]\s*(.*?)\s*$/,
    getNumber: (m) => Number(m[1]),
    getTitle: (m) => m[2]?.trim() || "",
  },
  {
  ...
  },
  {
    ...
    regex: /^\s*[-=*#~─━_]+\s*(?:제\s*)?(\d{1,5})\s*(?:화|장)\s*[-=*#~─━_]*\s*(.*?)\s*$/,
    getNumber: (m) => Number(m[1]),
    getTitle: (m) => m[2]?.trim() || "",
  },
  {
